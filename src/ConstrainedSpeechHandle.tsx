...
import { add, clamp, divide, getBoxCenter, multiply, rotate, subtract, translate } from "./util";
...
const ConstrainedSpeechHandle = ({ x, y, constraint, scene, setBounds, isTransforming }: Props) => {
    ...
    const point = { x: constraint === "x" ? verts[x].x : x, y: constraint === "y" ? verts[y].y : y };
    const inversePoint = { x: constraint === "x" ? verts[1 - x].x : x, y: constraint === "y" ? verts[1 - y].y : y };
    ...
    function endResize(event: React.MouseEvent) {
        ...
        const newVerts = correct(modifyVerts(modifyVerts(verts, bound, position, constraint), 2, getNewTail(position), constraint));
