...
import type { ModelCursor, VisualBlock, VisualCursor, VisualLine, VisualSpan, VisualText } from "./types";
...
export function squash(base?: Partial<BaseTextStyle>, block?: Partial<BaseTextStyle>, span?: Partial<BaseTextStyle>): BaseTextStyle {
...
export function buildBlocks(component: TextShape) {
    ...
    return component.blocks.map(modelBlock => {
        ...
        const visualBlock: VisualBlock = { lines: [], y, style: blockStyle, height: 0 };
        let line: VisualLine = { spans: [], y: offset, height: blockStyle.lineHeight };
        let lineWidth = 0;
        ...
        for (let spanIndex = 0; spanIndex < modelBlock.spans.length; spanIndex++) {
            ...
            for (let i = 0; i < tokens.length; i++) {
                ...
                if (!isSpace && lineWidth + currentWidth + tokenWidth > component.bounds.width) {
                    if (currentText.length > 0) {
                        line.spans.push({ text: currentText, charOffsets: generateOffsets(currentText), style: spanStyle, width: currentWidth, x: lineWidth, parentId: spanIndex, startIndex });
                    }

                    if (line.spans.length > 0) {
                        visualBlock.lines.push(line);
                        line = { spans: [], y: offset + visualBlock.lines.length * blockStyle.lineHeight, height: blockStyle.lineHeight };
                        lineWidth = 0;
                    }
...
export function toModel(cursor: VisualCursor, blocks: VisualText) {
    ...
    return { blockI: cursor.blockI, spanI: visualSpan.parentId, charI: visualSpan.startIndex + cursor.charI };
}
    ...
    for (let i = 0; i < visualBlock?.lines.length; i++) {
        ...
        for (let j = 0; j < line.spans.length; j++) {
            ...
            if (span.parentId === cursor.spanI) {
                const isFinalSpan = j === line.spans.length - 1 && i === visualBlock.lines.length - 1;
...
export function parseHit(pos: Vec2, blocks: VisualText, bounds: RelativeBounds) {
    ...
    const lineI = clamp1(Math.floor((relative.y - block.y) / block.style.lineHeight), 0, block.lines.length - 1);
    const line = block.lines[lineI];
    const spanI = scanLine(line, relative.x);
    const isFinalSpan = spanI === line.spans.length - 1 && lineI === block.lines.length - 1;
